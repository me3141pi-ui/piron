...
class Adam:
    ...
    def morph_over(self,wrapper):
        ...
        self.w_velocity = [jnp.zeros_like(layer.weights) for layer in layers]
        self.w_rms = [jnp.zeros_like(layer.weights) for layer in layers]
        self.b_velocity = [jnp.zeros_like(layer.bias) for layer in layers]
        self.b_rms = [jnp.zeros_like(layer.bias) for layer in layers]
    ...
    def update(self , grads):
        ...
        for i in range(len(w_grad)):
            ...
            w_step_scaled.append(w_velocity_hat/(jnp.sqrt(w_rms_hat)+self.epsilon))
            b_step_scaled.append(b_velocity_hat/(jnp.sqrt(b_rms_hat)+self.epsilon))
        self.time_step += 1
...
class RMSprop:
    ...
    def update(self, grads):
        ...
        for i in range(len(w_grad)):
            ...
            w_step_scaled.append(w_grad[i] / (jnp.sqrt(self.w_rms[i]) + self.epsilon))
            b_step_scaled.append(b_grad[i] / (jnp.sqrt(self.b_rms[i]) + self.epsilon))
...
class Nadam:
    ...
    def update(self, grads):
        ...
        for i in range(len(w_grad)):
            ...
            w_step_scaled.append(w_nadam / (jnp.sqrt(w_rms_hat) + self.epsilon))
            b_step_scaled.append(b_nadam / (jnp.sqrt(b_rms_hat) + self.epsilon))
        self.time_step += 1
        return w_step_scaled, b_step_scaled
