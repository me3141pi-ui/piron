...
class pillar:
    def __init__(self,input_dim,output_dim,activator = 'relu',dropout = 0):
        ...
        if type(activator) == str:
            ...
            try:
                self.activator , self.activator_derivative = activator
            except Exception:
                print('Invalid activator type encountered . Activator parameter must be string or function function_derivative tuple ')
        self.dropout = dropout
        #WEIGHTS AND BIASES
        self.weights = np.random.normal(0,1,(self.output_dim,self.input_dim))
        self.bias = np.random.normal(0,1,(self.output_dim,1))
        ...
        self.weight_grad_batch_sum = np.zeros_like(self.weights)
        self.bias_grad = np.zeros_like(self.bias)
        self.bias_grad_batch_sum = np.zeros_like(self.bias)
    ...
    def forward(self,prev_output):
        return self.activator(self.weights @ prev_output + self.bias)
